...
class sphy(pcrm.DynamicModel):
    def __init__(self):
        ...
        if self.PedotransferFLAG == 1:
            ...
            self.RootFieldMap = (
                pcr.readmap(self.inpath + config.get("SOIL", "RootFieldMap"))
                * self.RootFieldFrac
            )
            self.RootSatMap = (
                pcr.readmap(self.inpath + config.get("SOIL", "RootSatMap"))
                * self.RootSatFrac
            )
            self.RootDryMap = (
                pcr.readmap(self.inpath + config.get("SOIL", "RootDryMap"))
                * self.RootDryFrac
            )
            self.RootWiltMap = (
                pcr.readmap(self.inpath + config.get("SOIL", "RootWiltMap"))
                * self.RootWiltFrac
            )
            self.RootKsat = (
                pcr.readmap(self.inpath + config.get("SOIL", "RootKsat"))
                * self.RootKsatFrac
            )
            self.SubSatMap = pcr.readmap(self.inpath + config.get("SOIL", "SubSatMap"))
            self.SubFieldMap = pcr.readmap(
                self.inpath + config.get("SOIL", "SubFieldMap")
            )
        ...
        # -read the flag for precipitation correction
        ...
        if self.ETOpenWaterFLAG == 1:
            ...
            self.openWater = pcr.ifthenelse(
                self.openWaterFrac > 0, pcr.scalar(1), pcr.scalar(0)
            )
        ...
        if self.InfilFLAG == 1:
            ...
            try:
                ...
                self.Alpha = pcr.readmap(
                    self.inpath + config.get("INFILTRATION", "Alpha")
                )
            try:
        ...
        if self.SedFLAG == 1:
            ...
            self.RockFrac = pcr.readmap(
                self.inpath + config.get("SEDIMENT", "RockFrac")
            )
            ...
            if self.exclChannelsFLAG == 1:
                ...
                self.UpstreamArea = (
                    pcr.accuflux(self.FlowDir, 1) * pcr.cellarea() / 10**6
                )
    ...
    def initial(self):
        ...
        if not config.get("SOIL_INIT", "RootWater"):
            ...
            try:
                ...
                self.RootWater = pcr.readmap(
                    self.inpath + config.get("SOIL_INIT", "RootWater")
                )
        ...
        if not config.get("SOIL_INIT", "SubWater"):
            ...
            try:
                ...
                self.SubWater = pcr.readmap(
                    self.inpath + config.get("SOIL_INIT", "SubWater")
                )
        ...
        try:
            ...
            self.RootDrain = pcr.readmap(
                self.inpath + config.get("SOIL_INIT", "RootDrain")
            )
        ...
        if self.GroundFLAG == 1:
            ...
            try:
                ...
                self.SubDrain = pcr.readmap(
                    self.inpath + config.get("SOIL_INIT", "SubDrain")
                )
        ...
        for i in pars:
            if self.mm_rep_FLAG == 1 and eval("self." + i + "_mm_FLAG"):
                setattr(
                    self,
                    i + "SubBasinTSS",
                    pcrm.TimeoutputTimeseries(
                        i + "SubBasinTSS", self, self.Locations, noHeader=False
                    ),
                )
        ...
        if self.wbal_TSS_FLAG:
            self.wbalTSS = pcrm.TimeoutputTimeseries(
                "wbalTSS", self, self.Locations, noHeader=True
            )
            self.wbalTotTSS = pcrm.TimeoutputTimeseries(
                "wbalTotTSS", self, self.Locations, noHeader=True
            )
        self.waterbalanceTot = pcr.scalar(0.0)
    ...
    def dynamic(self):
        self.counter += 1
        print(
            str(self.curdate.day)
            + "-"
            + str(self.curdate.month)
            + "-"
            + str(self.curdate.year)
            + "  t = "
            + str(self.counter)
        )
        ...
        # -read the precipitation correction factor
        try:
        ...
        # -read the temperature correction factor
        try:
        ...
        if (
            self.mm_rep_FLAG == 1
            and self.Prec_mm_FLAG == 1
            and (self.RoutFLAG == 1 or self.ResFLAG == 1 or self.LakeFLAG == 1)
        ):
        ...
        if self.SnowFLAG == 1:
            ...
            Rain, self.SnowR, self.SnowSoil, OldTotalSnowStore = self.snow.dynamic(
                self,
                pcr,
                Temp,
                TempMax,
        ...
        self.reporting.reporting(
            self, pcr, "TotRainF", Rain * (1 - self.GlacFrac) + Rain_GLAC
        )  # for entire cell
        ...
        self.RootWater = self.RootWater + self.CapRise + self.SnowSoil
        ...
        self.RootWater = pcr.ifthenelse(
            RainFrac > 0, self.RootWater + Infil, self.RootWater
        )
        ...
        self.reporting.reporting(
            ...
            pcr,
            "TotETact",
            ETact * (1 - self.openWaterFrac) + self.ETOpenWater * self.openWaterFrac,
        )
        ...
        temp_rootperc = self.rootzone.RootPercolation(
        ...
        newdrain, newperc = self.rootzone.CalcFrac(
            pcr, self.RootWater, self.RootField, temp_RootDrain, temp_rootperc
        )
        ...
        self.reporting.reporting(
            self, pcr, "StorRootW", self.RootWater * (1 - self.openWaterFrac)
        )
        ...
        # -Report rootwater content
        self.reporting.reporting(
            self, pcr, "StorSubW", self.SubWater * (1 - self.openWaterFrac)
        )
        ...
        if self.LakeFLAG == 1 or self.ResFLAG == 1:
            ...
            Q = self.advanced_routing.dynamic(
                self, pcr, pcrm, config, TotR, self.ETOpenWater, PrecipTot
            )
        ...
        if self.GlacFLAG and self.GlacRetreat == 1:
            ...
            dS = (
                (
                    (self.RootWater - self.oldRootWater)
                    + (self.SubWater - self.oldSubWater)
                )
                * (1 - self.GlacFrac)
                + (self.Gw - self.oldGw)
                + (self.TotalSnowStore - OldTotalSnowStore)
                + (iceDepth - self.oldIceDepth)
            )
            ...
            dS = (
                (self.RootWater - self.oldRootWater)
                + (self.SubWater - self.oldSubWater)
            ) * (1 - self.GlacFrac) + (self.TotalSnowStore - OldTotalSnowStore)
