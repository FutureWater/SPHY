...
def initial(self, pcr, config):
    ...
    try:
        ...
        self.GwRecharge = pcr.readmap(
            self.inpath + config.get("GROUNDW_INIT", "GwRecharge")
        )
    # -initial baseflow
    try:
...
def dynamic(self, pcr, ActSubPerc, GlacPerc):
    ...
    self.reporting.reporting(
        self, pcr, "StorGroundW", self.Gw * (1 - self.openWaterFrac)
    )
    ...
    self.reporting.reporting(
        ...
        pcr,
        "GWL",
        ((self.SubDepthFlat + self.RootDepthFlat + self.GwDepth) / 1000 - self.H_gw)
        * -1,
    )
