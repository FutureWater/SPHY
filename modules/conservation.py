...
def init(self, pcr, config):
    try:
        ...
        self.input.input(self, config, pcr, 'conservationMeasures', 'SEDIMENT', 'conservationMeasures', 0)

        #-read table with conservation input parameters per conservation measure class
        pcr.setglobaloption('matrixtable')
        CONSERVATION_table = self.inpath + config.get('SEDIMENT', 'CONSERVATION_table')
        ...
        self.n_veg_TC_conservation = self.mmf.manningVegetation(self, pcr, self.d_field, self.Diameter_conservation, self.NoElements_conservation)
