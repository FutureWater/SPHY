    ...
    for i in pars:
        try:
            setattr(
                self,
                i + "RAold",
                pcr.readmap(self.inpath + config.get("ROUT_INIT", i + "RA_init")),
            )
...
def dynamic(self, pcr, pcrm, config, TotR, ETOpenWater, PrecipTot):
    ...
    self.StorRES = self.StorRES + pcr.ifthenelse(
        self.QFRAC == 0, 0.001 * pcr.cellarea() * TotR, 0
    )
    ...
    RunoffVolume = pcr.upstream(self.FlowDir, Qout) + pcr.ifthenelse(
        self.QFRAC == 0, 0, 0.001 * pcr.cellarea() * TotR
    )
    # -Routing of total flow
    tempvar = self.advanced_routing.ROUT(
    ...
    if self.ETOpenWaterFLAG == 1:
        ...
        ETaRES = pcr.ifthenelse(
            self.StorRES > 0,
            pcr.min(
                (
                    pcr.areatotal(
                        ETOpenWater * pcr.cellarea() * self.openWaterFrac,
                        self.openWaterNominal,
                    )
                    * 0.001
                ),
                self.StorRES,
        ...
        PrecipRES = pcr.ifthenelse(
            self.StorRES > 0,
            (
                pcr.areatotal(
                    PrecipTot * pcr.cellarea() * self.openWaterFrac,
                    self.openWaterNominal,
                )
                * 0.001
            ),
