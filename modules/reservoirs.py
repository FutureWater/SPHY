...
def QSimple(self, pcr):
    Q = pcr.max(
        pcr.min(
            self.ResKr * self.StorRES * (self.StorRES / self.ResSmax) ** self.ResB,
            self.StorRES,
        ),
...
def init(self, pcr, config):
    ...
    self.LocationsNoRes = pcr.ifthenelse(
        pcr.pcrand(pcr.scalar(self.Locations) > 0, pcr.scalar(self.ResID) == 0),
        pcr.boolean(1),
        pcr.boolean(0),
    )
    ...
    try:
        ...
        self.ResSmax = (
            pcr.lookupscalar(ResSimple_Tab, 3, self.ResID) * 10**6
        )  # convert to m3
        self.ResSimple = True
    except:
...
def initial_reporting(self, pcr, pcrm):
    ...
    self.ResETaTSS = pcrm.TimeoutputTimeseries(
        "ResETaTSS", self, self.ResID, noHeader=True
    )
    self.ResInCalTSS = pcrm.TimeoutputTimeseries(
        "ResInCalTSS", self, self.ResID, noHeader=True
    )
        ...
        if eval("self." + i + "RA_FLAG") and getattr(self, "Rep" + i + "_FLAG"):
            setattr(
                ...
                pcrm.TimeoutputTimeseries(
                    "Res" + i + "InTSS", self, self.ResID, noHeader=True
                ),
            )
            setattr(
                self,
                "Res" + i + "OutTSS",
                pcrm.TimeoutputTimeseries(
                    "Res" + i + "OutTSS", self, self.ResID, noHeader=True
                ),
            )
            setattr(
                self,
                "Res" + i + "StorTSS",
                pcrm.TimeoutputTimeseries(
                    "Res" + i + "StorTSS", self, self.ResID, noHeader=True
                ),
            )
