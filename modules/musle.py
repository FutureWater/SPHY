...
def MUSLE(self, pcr, Q_surf, q_peak):
    sed = (
        11.8
        * (Q_surf * q_peak * self.ha_area) ** 0.56
        * self.K_USLE
        * self.C_USLE
        * self.P_USLE
        * self.LS_USLE
...
def q_peak(self, pcr, Runoff):
    q_peak = (self.Alpha_tc * Runoff * pcr.cellarea()) / (
        3600000 * self.Tc
    )  # peak runoff in m3/s
    return q_peak
...
def LS_ULSE(self, pcr):
    ...
    LS = ((L_hill / 22.1) ** m) * (
        65.41 * pcr.sin(alpha_hill) ** 2 + 4.56 * pcr.sin(alpha_hill) + 0.065
    )
    return LS
...
def K_USLE(self, pcr):
    ...
    permeability = (
        permeability + pcr.scalar(pcr.pcrand(ksat_hourly > 50, ksat_hourly < 150)) * 2
    )
    permeability = (
        permeability + pcr.scalar(pcr.pcrand(ksat_hourly > 15, ksat_hourly < 50)) * 3
    )
    permeability = (
        permeability + pcr.scalar(pcr.pcrand(ksat_hourly > 5, ksat_hourly < 15)) * 4
    )
    permeability = (
        permeability + pcr.scalar(pcr.pcrand(ksat_hourly > 1, ksat_hourly < 5)) * 5
    )
    permeability = permeability + pcr.scalar(ksat_hourly < 1) * 6
    s = 2
    K_USLE = (
        (
            2.1 * 10**-4 * M_textural**1.14 * (12 - self.RootOMMap)
            + 3.25 * (s - 2)
            + 2.5 * (permeability - 3)
        )
        / 100
...
def Tc(self, pcr):
    slope_adjusted = pcr.ifthenelse(
        self.Slope < 0.0025, self.Slope + 0.0005, self.Slope
    )
