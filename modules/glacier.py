...
def init(self, pcr, config, pd, np, os):
    ...
    self.GlacTable = pd.read_csv(
        os.path.join(self.inpath, config.get("GLACIER", "GlacTable"))
    )
    cols = pd.DataFrame(
        columns=[
            "MOD_T",
            ...
            "OldSnowStore_GLAC",
            "SnowStore_GLAC",
            "OldSnowWatStore_GLAC",
            "SnowWatStore_GLAC",
            "MaxSnowWatStore_GLAC",
            "OldTotalSnowStore_GLAC",
            "TotalSnowStore_GLAC",
            "SnowR_GLAC",
            "GlacMelt",
            "GlacR",
    ...
    if self.GlacID_flag:
        self.glacid = sorted(
            self.GlacTable["GLAC_ID"].unique()
        )  # -get the unique glacier ids
        if (
            self.GlacID_memerror == 0
        ):  # -with no memory error we can store the pandas dataframes in the computer's memory
            drange = pd.date_range(self.startdate, self.enddate, freq="D")
            for (
                p
            ) in self.GlacVars:  # -make panda dataframes for each variable to report
                setattr(
                    self,
                    p + "_Table",
                    pd.DataFrame(index=drange, columns=self.glacid, dtype=np.float32),
                )  # -create table for each variable to report

    # -Read the glacier maps
    self.ModelID = pcr.pcr2numpy(
        pcr.readmap(os.path.join(self.inpath, config.get("GLACIER", "ModelID"))),
        self.MV,
    )  # -Nominal
    self.ModelID_1d = self.ModelID.flatten()  # -1 dim array with model cell IDs
    SelModelID = pd.unique(
...
def initial(self, pcr, pd):
    ...
    try:
        ...
        S = pd.DataFrame(
            data={"SnowStore_GLAC": SnowStore_1d[self.GlacierKeys]},
            index=self.ModelID_1d[self.GlacierKeys],
        )
    ...
    try:
        ...
        S = pd.DataFrame(
            data={"SnowWatStore_GLAC": SnowWatStore_1d[self.GlacierKeys]},
            index=self.ModelID_1d[self.GlacierKeys],
        )
        self.GlacTable.update(S)
        S = None
        SnowWatStore_1d = None
        del S, SnowWatStore_1d
    except:
        self.GlacTable["SnowWatStore_GLAC"] = self.SnowWatStore
    self.GlacTable["TotalSnowStore_GLAC"] = (
        self.GlacTable["SnowStore_GLAC"] + self.GlacTable["SnowWatStore_GLAC"]
    )
    # -Create table for accumulating glacier melt over certain period
    self.GlacTable["AccuGlacMelt"] = 0.0
    # -initialize the glacier fraction
    self.GlacFrac = np.zeros(self.ModelID_1d.shape)
    self.GlacFrac[self.GlacierKeys] = self.GlacTable.groupby(
        self.GlacTable.index
    ).sum()["FRAC_GLAC"]
    self.GlacFrac = self.GlacFrac.reshape(self.ModelID.shape)
    self.GlacFrac = pcr.numpy2pcr(pcr.Scalar, self.GlacFrac, self.MV)
    self.GlacFrac = pcr.ifthen(
        self.clone, self.GlacFrac
    )  # -only use values where clone is True
    pcr.report(
        self.GlacFrac,
        self.outpath + "GlacFrac_" + self.curdate.strftime("%Y%m%d") + ".map",
    )
    ...
    if self.GlacRetreat == 1:
        ...
        GlacTable_MODid["ICE_DEPTH"] = (
            GlacTable_MODid["ICE_DEPTH"] * GlacTable_MODid["FRAC_GLAC"]
        )
        ...
        pcr.report(
            glacArea,
            self.outpath + "GlacArea_" + self.curdate.strftime("%Y%m%d") + ".map",
        )
        ...
        pcr.report(
            iceDepth,
            self.outpath + "iceDepth_" + self.curdate.strftime("%Y%m%d") + ".map",
        )
        self.oldIceDepth = iceDepth * 1000  # in mm
        # -Report pcraster map of glacier volume
        pcr.report(
            iceDepth * glacArea,
            self.outpath + "vIce_" + self.curdate.strftime("%Y%m%d") + ".map",
        )
        # -Delete variables that are not needed
        glacArea = None
        del glacArea
        # iceDepth = None; del iceDepth;
...
def dynamic(self, pcr, pd, Temp, Precip):
    ...
    T = pd.DataFrame(
        data={"MOD_T": T_1d[self.GlacierKeys]}, index=self.ModelID_1d[self.GlacierKeys]
    )
    ...
    P = pd.DataFrame(
        data={"Prec_GLAC": P_1d[self.GlacierKeys]},
        index=self.ModelID_1d[self.GlacierKeys],
    )
    ...
    # -Snow runoff
    mask = (
        self.GlacTable["SnowWatStore_GLAC"] == self.GlacTable["MaxSnowWatStore_GLAC"]
    ) & (
        self.GlacTable["OldSnowStore_GLAC"] > 0.0
    )  # -mask with true where SnowWatStore_GLAC == MaxSnowWatStore_GLAC
    self.GlacTable.loc[mask, "SnowR_GLAC"] = (
        self.GlacTable.loc[mask, "ActSnowMelt_GLAC"]
        + self.GlacTable.loc[mask, "Rain_GLAC"]
        - (
            self.GlacTable.loc[mask, "SnowWatStore_GLAC"]
    ...
    )  # -mask with true where snowpack has melted within timestep: for these cells also partial glacier melt
    fullMelt = (self.GlacTable["OldSnowStore_GLAC"] == 0) & (
        self.GlacTable["SnowStore_GLAC"] == 0
    )
    # -Melt from Clean Ice Glaciers
    mask = partialMelt & self.CImask  # -mask for Clean Ice glacier and partial melt
    self.GlacTable.loc[mask, "GlacMelt"] = (
        np.maximum(
            self.GlacTable.loc[mask, "GLAC_T"]
            - (self.GlacTable.loc[mask, "OldSnowStore_GLAC"] / self.DDFS),
            0,
        )
        * self.DDFG
    )
    ...
    mask = (
        partialMelt & self.DBmask
    )  # -mask for Debris covered Glacier and partial melt
    self.GlacTable.loc[mask, "GlacMelt"] = (
        np.maximum(
            self.GlacTable.loc[mask, "GLAC_T"]
            - (self.GlacTable.loc[mask, "OldSnowStore_GLAC"] / self.DDFS),
    ...
    self.GlacTable["AccuGlacMelt"] = (
        self.GlacTable["AccuGlacMelt"] + self.GlacTable["GlacMelt"]
    )
    # -Glacier runoff
    mask = (
        self.GlacTable["OldSnowStore_GLAC"] == 0
    )  # -only add rain to glacmelt when there was no snowpack at beginning to time-step
    # -Glacier runoff consisting of melt and rainfall
    self.GlacTable.loc[mask, "GlacR"] = self.GlacF * (
        self.GlacTable.loc[mask, "GlacMelt"] + self.GlacTable.loc[mask, "Rain_GLAC"]
    )
    # -Glacier percolation consisting of melt and rainfall
    self.GlacTable.loc[mask, "GlacPerc"] = (1 - self.GlacF) * (
        self.GlacTable.loc[mask, "GlacMelt"] + self.GlacTable.loc[mask, "Rain_GLAC"]
    )
    # -Glacier runoff consisting of melt only
    self.GlacTable.loc[np.invert(mask), "GlacR"] = (
        self.GlacF * self.GlacTable.loc[np.invert(mask), "GlacMelt"]
    )
    # -Glacier percolation consisting of melt only
    self.GlacTable.loc[np.invert(mask), "GlacPerc"] = (
        1 - self.GlacF
    ) * self.GlacTable.loc[np.invert(mask), "GlacMelt"]
    mask = None
    del mask

    # -Aggregation for model grid ID
    GlacTable_MODid = self.GlacTable.loc[
        :,
        [
            "Rain_GLAC",
            "Snow_GLAC",
    ...
    GlacTable_MODid = GlacTable_MODid.multiply(
        self.GlacTable["FRAC_GLAC"], axis="index"
    )  # -Multiply with the glacier fraction
    GlacTable_MODid = GlacTable_MODid.groupby(
        GlacTable_MODid.index
    ).sum()  # -Summarize by model ID
    ...
    self.TotalSnowStore_GLAC = pcr.numpy2pcr(
        pcr.Scalar, self.TotalSnowStore_GLAC, self.MV
    )
    # -SnowR from glacier
    ...
    if self.mm_rep_FLAG == 1 and (
        ...
        self.GMeltSubBasinTSS.sample(
            pcr.catchmenttotal(GlacMelt, self.FlowDir)
...
def dynamic_reporting(self, pcr, pd, np):
    ...
    if self.GlacID_flag:
        ...
        GlacTable_GLACid = GlacTable_GLACid.multiply(
            self.GlacTable["FRAC_GLAC"], axis="index"
        )  # -Multiply with fraction
        GlacTable_GLACid["GLAC_ID"] = self.GlacTable["GLAC_ID"]
        GlacTable_GLACid["FRAC_GLAC"] = self.GlacTable[
            "FRAC_GLAC"
        ]  # -Add GLAC_ID column and FRAC_GLAC column
        GlacTable_GLACid = GlacTable_GLACid.groupby(
            "GLAC_ID"
        ).sum()  # -Summarize by Glacier ID
        FracSum = GlacTable_GLACid["FRAC_GLAC"]  # -Get summed glacier fraction
        GlacTable_GLACid = GlacTable_GLACid.div(
            FracSum, axis="index"
        )  # -Divide by total fraction for glacier weighted average (frac==1)
        GlacTable_GLACid["FRAC_GLAC"] = FracSum
        FracSum = None
        del FracSum
        GlacTable_GLACid = (
        ...
        if self.GlacID_memerror == 0:
            for v in self.GlacVars:
                vv = getattr(self, v + "_Table")
                vv.loc[self.curdate, :] = GlacTable_GLACid.loc[v, :].astype(np.float32)
            v = None
            vv = None
            del v, vv
            GlacTable_GLACid = None
            del GlacTable_GLACid
            if (
                self.curdate == self.enddate
            ):  # -do the reporting at the final model time-step
                for v in self.GlacVars:
            ...
            for v in self.GlacVars:
                ...
                if self.curdate == self.startdate:
                    ...
                    df.to_csv(
                        self.outpath + v + ".csv", mode="a", header=False
                    )  # -no header for time-step > 1
            df = None
            del df
            GlacTable_GLACid = None
            del GlacTable_GLACid
    ...
    if (
        self.GlacRetreat == 1
        and self.curdate.month == self.GlacUpdate["month"]
        and self.curdate.day == self.GlacUpdate["day"]
    ):
        ...
        GlacFracTable["dMelt"] = (
            GlacFracTable["TotalSnowStore_GLAC"] - GlacFracTable["AccuGlacMelt"]
        )
        GlacFracTable["dMelt"] = (
        ...
        ablMask = GlacFracTable["dMelt"] < 0.0
        # -Set the ablation and accumulation in the corresponding fields
        GlacFracTable["Accumulation"] = 0.0
        GlacFracTable["Ablation"] = 0.0
        GlacFracTable.loc[np.invert(ablMask), "Accumulation"] = GlacFracTable.loc[
            np.invert(ablMask), "dMelt"
        ]
        GlacFracTable.loc[ablMask, "Ablation"] = GlacFracTable.loc[ablMask, "dMelt"]
        # -Set the ice volumes for the ablation UIDs
        GlacFracTable["V_ice_ablation"] = 0.0
        GlacFracTable.loc[ablMask, "V_ice_ablation"] = GlacFracTable.loc[
            ablMask, "V_ice_t0"
        ]
        # -Calculate totals per Glacier ID
        GlacID_grouped = GlacFracTable.groupby("GLAC_ID").sum()
        GlacID_grouped = GlacID_grouped.loc[
            :, ["dMelt", "Accumulation", "V_ice_ablation"]
        ]

        # -Calculate total delta Melt (dMelt), accumulation, and ice volumes (of ablation cells) for each glacier ID
        GlacFracTable["dMelt_group"] = 0.0
        GlacFracTable["Accumulation_group"] = 0.0
        GlacFracTable["V_ice_ablation_group"] = 0.0

        for index, row in GlacID_grouped.iterrows():
            GlacFracTable.loc[GlacFracTable["GLAC_ID"] == index, "dMelt_group"] = row[
                "dMelt"
            ]
            GlacFracTable.loc[
                GlacFracTable["GLAC_ID"] == index, "Accumulation_group"
            ] = row["Accumulation"]
            GlacFracTable.loc[
                GlacFracTable["GLAC_ID"] == index, "V_ice_ablation_group"
            ] = row["V_ice_ablation"]
        ...
        negDistMask = (GlacFracTable["dMelt_group"] < 0.0) & (
            GlacFracTable["dMelt"] >= 0.0
        )
        posDistMask = (GlacFracTable["dMelt_group"] < 0.0) & (
            GlacFracTable["dMelt"] < 0.0
        )
        # -Calculate the ice redistribution
        GlacFracTable["Ice_redist"] = 0.0
        ...
        GlacFracTable["V_ice_t1"] = (
            GlacFracTable["V_ice_t0"]
            + GlacFracTable["Accumulation"]
            + GlacFracTable["Ablation"]
        ...
        GlacFracTable["V_ice_positive"] = np.maximum(0.0, GlacFracTable["V_ice_t1"])
        # -Calculate totals per Glacier ID
        GlacID_grouped = GlacFracTable.groupby("GLAC_ID").sum()
        GlacID_grouped = GlacID_grouped.loc[:, ["V_ice_negative", "V_ice_positive"]]
        GlacFracTable["V_ice_negative_group"] = 0.0
        GlacFracTable["V_ice_positive_group"] = 0.0
        for index, row in GlacID_grouped.iterrows():
            GlacFracTable.loc[
                GlacFracTable["GLAC_ID"] == index, "V_ice_negative_group"
            ] = row["V_ice_negative"]
            GlacFracTable.loc[
                GlacFracTable["GLAC_ID"] == index, "V_ice_positive_group"
            ] = row["V_ice_positive"]
        # -Remove GlacID_grouped table
        GlacID_grouped = None
        del GlacID_grouped
        ...
        GlacFracTable.loc[noIceMask, "FRAC_GLAC_new"] = 0.0
        noIceMask = None
        del noIceMask
        ...
        self.GlacTable["SnowStore_GLAC"] = 0.0
        self.GlacTable["SnowWatStore_GLAC"] = 0.0
        self.GlacTable["TotalSnowStore_GLAC"] = 0.0
    ...
    if self.GlacRetreat == 1 and self.curdate == self.dateAfterUpdate:
        ...
        glac_csv = self.GlacTable.loc[
            :,
            ["U_ID", "GLAC_ID", "MOD_H", "GLAC_H", "DEBRIS", "FRAC_GLAC", "ICE_DEPTH"],
        ]
        glac_csv.insert(1, "MOD_ID", self.GlacTable.index)
        glac_csv.to_csv(
            self.outpath + "glacTable_" + self.curdate.strftime("%Y%m%d") + ".csv",
            index=False,
        )
        glac_csv = None
        del glac_csv
        ...
        GlacTable_MODid["AREA"] = GlacTable_MODid["FRAC_GLAC"] * self.cellArea
        GlacTable_MODid = GlacTable_MODid.groupby(GlacTable_MODid.index).sum()
        GlacTable_MODid.fillna(0.0, inplace=True)
