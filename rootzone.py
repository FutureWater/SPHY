...
def RootRunoff(self, pcr, rainfrac, rain):
    ...
    if self.InfilFLAG == 1:
        ...
        Infil_cap = (
            ...
            * self.RootKsat
            / 24
            * (1 + ((self.RootSat - self.RootWater) / self.RootSat)) ** self.Labda_Infil
        )

        # -Infiltration
        Infil_excess = pcr.ifthenelse(
            (self.Alpha * rain) > Infil_cap,
            rain - ((self.Alpha * rain - Infil_cap) ** 2) / (self.Alpha**2 * rain),
            rain,
        )
        Saturated_excess = self.RootSat - self.RootWater
        Infil = pcr.max(0, pcr.min(Infil_excess, Saturated_excess)) * (
            1 - self.pavedFrac
        )
...
def RootPercolation(pcr, rootwater, subwater, rootfield, rootTT, subsat):
    ...
    rootperc = pcr.ifthenelse(
        subwater >= subsat, 0, pcr.min(subsat - subwater, rootperc)
    )
